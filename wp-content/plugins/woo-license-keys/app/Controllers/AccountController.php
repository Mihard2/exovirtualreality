...
class AccountController extends Controller
{
    ...
    /**
     * Retuns endpoint title.
     * @since 1.0.0
     * 
     * @hook the_title
     *
     * @global object $wp_query Wordpress query string manager.
     *
    ...
    public function title( $title )
    {
        ...
        if ( ! is_admin()
            ...
            if ( isset( $wp_query->query_vars[ self::ENDPOINT ] ) ) {
                $title = __( 'License Keys', 'woo-license-keys' );
                remove_filter( 'the_title', [ $this, 'title' ] );
            } else if ( isset( $wp_query->query_vars[ self::VIEW_ENDPOINT ] ) ) {
                $title = __( 'License Key', 'woo-license-keys' );
                remove_filter( 'the_title', [ $this, 'title' ] );
            }
        }
        return $title;
    ...
    public function view_enqueue()
    {
        ...
        wp_enqueue_script(
            'clipboard',
            assets_url( 'js/clipboard.min.js', __DIR__ ),
            [],
            '2.0.4',
            true
        );
        wp_enqueue_script(
            ...
            ['clipboard', 'jquery'],
            $licensekeys->config->get( 'version' ),
            true
        );
    }
}