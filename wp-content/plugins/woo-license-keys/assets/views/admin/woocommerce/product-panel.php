...
jQuery(document).ready(function() {
    if (jQuery( '#_expire' ).length !== 0) {
        /**
         * Enable pricing.
         * @since 1.0.0
         * @link http://jeroensormani.com/adding-a-custom-woocommerce-product-type/
         */
        jQuery( '.options_group.pricing' ).addClass( '<?= wc_lk_show_if( $types ) ?>' ).show();
        if ( jQuery( '#product-type' ).length > 0 && jQuery( '#product-type' ).val() === 'license_key' ) {
         ...
         * @since 1.0.0
         */
        jQuery( 'label[for="_virtual"]' ).addClass( '<?= wc_lk_show_if( $types ) ?>' ).show();
        /**
         * Enable downloadable option.
         * @since 1.0.0
         */
        jQuery( 'label[for="_downloadable"]' ).addClass( '<?= wc_lk_show_if( $types ) ?>' ).show();
        /**
         * Disable expire interval and value when not selected.
         * @since 1.0.0
         */
        if (jQuery( '#_expire' ).attr( 'checked' ) === undefined) {
            jQuery( '._expire_interval_field' ).hide();
            jQuery( '._expire_value_field' ).hide();
            jQuery( '._expire_notifications_group' ).hide();
        }
        /**
         * Bind to press event on expire checkbox.
         * @since 1.0.0
         */
        jQuery( '#_expire' ).click(function() {
            if (jQuery( '#_expire' ).attr( 'checked') ) {
                jQuery( '._expire_interval_field' ).show();
                jQuery( '._expire_value_field' ).show();
                jQuery( '._expire_notifications_group' ).show();
            } else {
                jQuery( '._expire_interval_field' ).hide();
                jQuery( '._expire_value_field' ).hide();
                jQuery( '._expire_notifications_group' ).hide();
